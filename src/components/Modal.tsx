  ...
  return (
    <div className="flex justify-center items-center absolute top-0 left-0 right-0 bottom-0 bg-black/70 z-30">
      <dialog open={isModalOpen} className="bg-white p-4 rounded-md">
        ...
        {isRegistering ? (
          <form className="flex flex-col gap-2 mt-2" onSubmit={handleCreateAccount}>
            <div className="flex flex-col gap-1">
              <label htmlFor="name">Name</label>
              <input className="border p-2" type="text" name="name" id="name" placeholder="Alejandro" required value={name} onChange={(e) => setName(e.target.value)} />
            </div>
            <div className="flex flex-col gap-1">
            ...
            <button disabled={loading} className="bg-blue-400 p-2 rounded-md w-full text-white mt-4 hover:bg-blue-500">Crear Cuenta</button>
